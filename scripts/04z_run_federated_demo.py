...
def main():
    ...
    for b in BANKS:
        run(PY + ["scripts/04a_client_report_stats.py", "--bank", b])
    ...
    for r in range(1, NUM_ROUNDS + 1):
        for b in BANKS:
            run(PY + ["scripts/04c_client_train_round.py", "--bank", b, "--round", str(r), "--local_epochs", "2"])
        run(PY + ["scripts/04d_server_aggregate_round.py", "--round", str(r)])

    # evaluate best
    for b in BANKS:
        run(PY + ["scripts/04e_client_eval_best.py", "--bank", b])
